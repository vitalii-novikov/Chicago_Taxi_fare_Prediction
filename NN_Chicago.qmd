...
```{r}
taxi_data_half2024 <- readr::read_csv("chicago_taxi_half2024.csv")
```
## explore dataset

```{r}
taxi_data_half2024 %>% head()
```
...
taxi_data_half2024 %>% select(all_of(numeric_columns)) %>% summary()
```
## Filter zero values

```{r}
df <- taxi_data_half2024 %>% 
